...
import { SectorLoader } from './sector-loader.js';
...
export class Importer {
    ...
    initUI(sceneDirectory) {
        if (game.user?.isGM) {
            ...
            `;
            const html = $(sceneDirectory); // convert to jQuery
    ...
    async importFile(fileName, options) {
        ...
        if (game.user?.isGM) {
            try {
                const importedData = await (await fetch(fileName)).json();
